...
## Décision
Un service gRPC `GameStats` exposant une unique RPC `GetStats`, consommé depuis Blazor via gRPC-Web (bouton « Statistiques » sur la page de jeu). Le contrat est défini dans [Protos/battleship.proto](../../Protos/battleship.proto), partagé par l'API et le front.
...
Démonstration manuelle : créer une partie dans l'interface, cliquer sur « Statistiques (gRPC) » (succès), puis appeler le même service avec un identifiant de partie aléatoire (via`grpcurl` nous on a fait comme ça direct sur l'API) pour observer l'erreur `NotFound`
